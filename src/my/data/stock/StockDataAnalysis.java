...
public abstract class StockDataAnalysis <T extends StockMetaData> {
	...
	/**
	 * constructor
	 * @param stockDataObject
	 * @throws Exception
	 */
	public StockDataAnalysis(T stockDataObject) throws Exception{
		T.getInstance();
	...
	// for multi thread Analysis
		...
		public void Analysis() throws Exception {
			...
			// start Analysiss for all stockcodes
			this.Analysis(T.getStockCodes());
		...
		public void Analysis(String... stockcodes) throws NumberFormatException, Exception {
			//
...
//						+ xLocalFilePath);
		...
		public void luanchThread(String ...sub_scs) throws Exception{
			StockDataAnalysisThread xThread = new StockDataAnalysisThread(sub_scs);	//init thread
		...
		class StockDataAnalysisThread
			...
			public StockDataAnalysisThread(String[] stockcodes) throws Exception {
			...
			public void run() {
				...
				for (String xsc : this._stockcodes) {
					...
					try {
						sc_urls_localFiles = SotckDataAnalysis.this.getURL_File(xsc);
					} catch (Exception e1) {
						e1.printStackTrace();
					}

					try {
						SotckDataAnalysis.this.Analysis(xsc, sc_urls_localFiles);
					} catch (Exception e) {
						e.printStackTrace();
					}
				}
				...
				SotckDataAnalysis.this.removeCompletedThread();
